...
#include "PlaidWireFunctions.h" /* Function declarations */
...
void PlaidWire_Use(CommandContext context, GameState* gameState, WorldData* worldData)
{
	...
	if (gameState->currentRoomIndex != 30)
	{
		...
		printf("There is no PLAID WIRE here.\n");
		return;
	}

	/* check if the cage has already been broken and scored */
	if (GameFlags_IsInList(gameState->gameFlags, "plaidSolution"))
	{
...
Item* PlaidWire_Build()
{
	/* Create a "brick" item, using the functions defined in this file */
	return Item_Create("plaid wire", "AAAAAAAAAAHHHHHHHHH", true, PlaidWire_Use, NULL, NULL);
}